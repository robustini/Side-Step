...
```powershell
cd Side-Step
uv run sidestep              # Interactive wizard
uv run sidestep gui          # Desktop GUI
uv run sidestep train ...    # Direct CLI
# Or just `sidestep ...` if the command is on your PATH
```
...
## Electron GUI Recovery

If `sidestep gui` fails with an error like:

```
Cannot find module ...\frontend\electron\node_modules\electron\cli.js
```

your local Electron install is incomplete (usually from an interrupted `npm install`).
On Windows this can happen even when `node_modules\.bin\electron.cmd` exists.

Reinstall the local Electron shell from the project root:

```powershell
Set-Location frontend/electron; npm install --no-fund --no-audit
```
...
Side-Step will automatically fall back to pywebview or your browser if Electron is unavailable.
