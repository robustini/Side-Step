...
> **GUI users:** The Live VRAM Estimation bar in Advanced Mode shows model + activation + optimizer breakdown before you start training. It updates reactively as you adjust settings. VRAM presets (8 GB, 12 GB, 16 GB, 24 GB+) are available in both the GUI and the wizard.
...
**How to disable:** In the wizard or GUI (Advanced mode), disable gradient checkpointing. Via CLI: `--no-gradient-checkpointing`.

**Partial checkpointing:** Side-Step also supports `--gradient-checkpointing-ratio` (default: 1.0). Set to 0.5 to only checkpoint half the decoder layers -- a middle ground between full checkpointing and none.
...
**How to enable:** Select "AdamW 8-bit" in the wizard or GUI optimizer dropdown. Via CLI: `--optimizer-type adamw8bit`. The `bitsandbytes` package is included automatically. **This is now the default optimizer.**
...
**How to enable:** Enable in the wizard or GUI VRAM section. Via CLI: `--offload-encoder`. **This is now ON by default.** Use `--no-offload-encoder` to disable if you have VRAM to spare and want to keep everything on GPU.
...
**Trade-off:** Lower capacity. The adapter can capture less of your dataset's style. See the rank guide in [[Training Guide]]. Consider using [[Preprocessing++]] to assign adaptive ranks -- it puts capacity where it matters.
...
| **Standard** | 16-24 GB | Grad checkpointing ON, Flash Attention | Batch 1, AdamW8bit, Rank 64, Grad accumulation 4 |
| **Tight** | 12-16 GB | Grad checkpointing ON, Flash Attention | Batch 1, **AdamW8bit**, Encoder offloading, Rank 32-64 |
| **Minimal** | 8-12 GB | Grad checkpointing ON, Flash Attention | Batch 1, **AdamW8bit**, Encoder offloading, Rank 16, High grad accumulation |
...
- [[Training Guide]] -- Full training workflow, hyperparameter guide, adapter types
- [[Preprocessing++]] -- Adaptive rank assignment for targeted training
- [[Getting Started]] -- Installation and first-run setup
- [[Preset Management]] -- Built-in VRAM presets and configuration management
- [[CLI Argument Reference]] -- All VRAM-related CLI flags
