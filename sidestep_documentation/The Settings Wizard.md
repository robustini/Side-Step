...
| Dropout | `0.1` | Dropout on LoRA/DoRA layers. Prevents overfitting | Increase (0.2-0.3) for very small datasets, decrease (0.05) for large ones |
| Attention type | `both` | Which attention layers get adapters: self, cross, or both | `self` = audio patterns only, `cross` = text conditioning only. See [[Preprocessing++]] to decide |
| Target projections | `q_proj k_proj v_proj o_proj` | Which projections inside each attention block | Use [[Preprocessing++]] results to narrow this down |
...
#### LoHA Settings (when training LoHA)

| Setting | Default | What it does | Why you'd change it |
|---|---|---|---|
| Linear dimension | `64` | Analogous to LoRA rank | Same guidance as LoRA rank |
| Linear alpha | `128` | Scaling factor | Keep at 2x linear dim |
| Factor | `-1` | Hadamard factorization factor (-1 = auto) | Leave at auto |
| Tucker decomposition | `no` | Use Tucker decomposition | Alternative factorization -- experimental |
| Scalar scaling | `no` | Use scalar scaling | Experimental |

#### OFT Settings (when training OFT) -- Experimental

| Setting | Default | What it does | Why you'd change it |
|---|---|---|---|
| Block size | `64` | OFT block size (replaces rank as the capacity knob) | Lower for less capacity, higher for more |
| Constrained OFT | `no` | Enable Cayley projection for strict orthogonality | May improve stability |
| Epsilon | `6e-5` | Numerical stability constant | Rarely needs changing |
...
| Setting | Default | What it does | Why you'd change it |
|---|---|---|---|
| Save every N epochs | `50` | Full checkpoint (adapter + optimizer + scheduler) | Lower for safety on long runs, higher to save disk space |
