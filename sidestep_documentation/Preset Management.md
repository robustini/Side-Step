...
A preset stores training fields covering adapter type, adapter-specific settings, training hyperparameters, and VRAM-related options. It does **not** store:
...
| LoRA / DoRA | `rank`, `alpha`, `dropout`, `target_modules_str`, `attention_type`, `bias` |
...
| LoHA | `loha_linear_dim`, `loha_linear_alpha`, `loha_factor`, `loha_use_tucker`, `loha_use_scalar` |
| OFT | `oft_block_size`, `oft_coft`, `oft_eps` |
| Training | `learning_rate`, `batch_size`, `gradient_accumulation`, `epochs`, `warmup_steps`, `weight_decay`, `max_grad_norm`, `seed`, `optimizer_type`, `scheduler_type`, `cfg_ratio` |
| Checkpointing / Logging | `save_every`, `log_every`, `log_heavy_every` |
| VRAM | `gradient_checkpointing`, `gradient_checkpointing_ratio`, `offload_encoder` |
...
| 3 (lowest) | `assets/presets/` | All | Built-in presets shipped with Side-Step. Read-only. |
...
The local directory is anchored to the Side-Step project root (found by looking for `train.py` or `pyproject.toml`), so presets are always found regardless of your current working directory.
